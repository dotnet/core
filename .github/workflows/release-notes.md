...
Your outputs are pull requests — a **set per active milestone**. Each milestone produces:

1. A **base PR** containing `changes.json`, `features.json`, `README.md`, and `build-metadata.json` for the milestone, targeting `main`.
2. A **per-component PR** for each component file (`aspnetcore.md`, `runtime.md`, `libraries.md`, etc.), each targeting the milestone's base branch.

When a component PR merges into the base branch, the base PR's diff grows; the base PR is the consolidation. See `pr-layout.md` for the full layout, branch naming, PR titles, and draft-state rules.

The artifacts in each PR are:

1. **`changes.json`** (base PR) — a comprehensive manifest of all PRs/commits that shipped, generated by `release-notes generate changes`
2. **`features.json`** (base PR) — a scored derivative of `changes.json` used to rank what is worth documenting
3. **Markdown release notes** (component PRs) — curated editorial content covering high-value features, one file per component PR
...
- **component-mapping.md** — VMR paths → components → output files → branch suffixes → default assignees
- **pr-layout.md** — base + per-component branches, PR titles, draft-state rules
...
4. **Check for an existing branch set** on this repo (search for `release-notes/11.0-previewN*` branches and any `[release-notes]` PRs matching the milestone)

Each milestone gets its own branch set on this repo: a base branch plus one branch per component file. See `pr-layout.md`.
...
Process milestones in order (lowest to highest). Each gets its own branch set and PR set per `pr-layout.md`.

#### a. Regenerate changes.json

Regenerate when the milestone is new or when the head ref has moved since the previous run. If `build-metadata.json` already exists on the base branch and its `head` ref matches what you would generate against now, skip regeneration and reuse the existing `changes.json`. Always regenerate when starting a milestone for the first time.
...
#### d. Check for human edits on the branch set

If a PR set already exists, walk every branch:

```bash
# Base branch
git log --oneline --author!=github-actions origin/release-notes/11.0-preview4

# Each component branch
for c in aspnetcore runtime libraries sdk csharp fsharp efcore winforms wpf msbuild nuget containers dotnetmaui; do
  git log --oneline --author!=github-actions "origin/release-notes/11.0-preview4-$c" 2>/dev/null
done
```

Identify which markdown files humans have edited, on which branches. For those files, diff them to understand what changed. Do NOT overwrite human-edited sections. Only add new sections or update sections the agent previously wrote that no human has touched. Each component file is only ever edited on its own branch.

If branches already have drafted markdown, that content is the **baseline** for the next run. Follow the shared `update-existing-branch` playbook: refresh `changes.json` only if the preview moved forward (on the base branch), merge the delta into `features.json` (on the base branch), preserve the current structure, address unresolved feedback, and update only the sections affected by new evidence or newly shipped changes (on the relevant component branches).
...
- Route each output file to the matching component branch per `pr-layout.md` — `aspnetcore.md` only ever lives on `release-notes/{version}-{slug}-aspnetcore`, `runtime.md` only on `…-runtime`, etc.
...
- Components with no noteworthy changes get a minimal stub (still on their own branch / PR)
...
Check all comments and review threads on the PR set (base PR + every component PR) since the last run:
...
#### i. Create or update the PR set

Follow `pr-layout.md` for branch names, PR titles, draft-state rules, default assignees from `component-mapping.md`, and the seed order when the safe-output cap is hit.

There are three cases. Decide which by listing remote branches matching `release-notes/11.0-preview4*` and the open `[release-notes]` PRs for the milestone:

- **No branch set exists.** Create the base branch `release-notes/11.0-preview4` off `main` with `changes.json`, `features.json`, `README.md`, and `build-metadata.json`. Open the base draft PR targeting `main`. Then create per-component branches off the base branch and open their draft PRs **in the seed order from `pr-layout.md`** (`aspnetcore`, `runtime`, `libraries`, `sdk`, then the rest). Stop opening new PRs when the remaining `safe-outputs.create-pull-request` budget is exhausted; the next scheduled run will pick up where you left off.

- **Partial branch set exists.** The base branch (and base PR) exist, but one or more expected component branches/PRs do not. The expected component set is the standard set from `component-mapping.md` (skip `visualbasic` unless VB-specific changes exist; out-of-VMR components like `containers` and `dotnetmaui` still get branches/PRs even when their content sources are external). For each expected component without a branch/PR, create it now in seed order, again respecting the per-run cap.

- **Complete branch set exists.** Push refreshed metadata to the base branch only when the head ref moved. Push refreshed component markdown only to component branches that actually changed in this run (driven by review feedback or by features that newly landed/were rescored). Do not blindly touch every component branch — that wastes the `push-to-pull-request-branch.max` budget. Do not modify a component file from the wrong branch and do not modify metadata from a component branch. Comment on each touched PR summarizing what changed.

When metadata on the base branch changes, GitHub recalculates the diff for component PRs targeting the base, but the component branch working trees remain stale. Only merge/rebase base into a component branch when you are about to edit that component's markdown and need the refreshed metadata.

PR title format:

- Base PR: `[release-notes] .NET 11 Preview 4`
- Component PR: `[release-notes] {Component name} in .NET 11 Preview 4`

PR bodies should summarize: milestone, number of changes (base PR), which features are covered (component PRs), and any open questions or items needing human review.
...
- **Base PR was merged** — the milestone is done. Skip it on future runs.
- **Base PR was closed without merging** — something went wrong at the milestone level. Don't reopen. Log it and move on.
- **Component PR was merged** — that component is done for the milestone. Don't reopen its branch. The base PR's diff already reflects the merge.
- **Component PR was closed without merging** — assume a human did this on purpose (e.g. nothing notable shipped for that component). Don't reopen. Mark that component complete for the milestone.
- **Component PR is missing** — if it's missing because a previous run hit the safe-output cap, create it now (see "Partial branch set exists" above). If it's missing because no PR was ever opened and no closure record exists, treat that as the partial case as well.

### 4. Daily summary

At the end of each run, leave a comment on each active PR (base and components) noting:
