...
compatibility: Requires GitHub MCP server or gh CLI for cross-repo queries. Uses dotnet-release generate changes for structured change data. Uses dotnet-inspect for API verification.
---
...
2. The agent reads `changes.json` and identifies high-value features worth documenting
3. The agent uses `dotnet-inspect` to verify public API names, type shapes, and member signatures before writing about them
4. The agent writes curated markdown release notes with verified API references and code samples
