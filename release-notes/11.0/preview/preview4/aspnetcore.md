...
A new `blazor-wasm-servicedefaults` template scaffolds a service-defaults library for Blazor WebAssembly clients for integration with Aspire ([dotnet/aspnetcore #64807](https://github.com/dotnet/aspnetcore/pull/64807)). It mirrors the service-defaults pattern that Aspire already uses for server projects and `dotnet/maui` ships for MAUI clients.
