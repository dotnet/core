...
1. Design time build failure and dependency node warning icons when targeting *netcoreapp2.1* and specifying a *RuntimeIdentifier* in the project file.  This is the default for Service Fabric projects.
   ...
   **Workaround**
   ...
   ```
2. Starting with .NET 5 preview 8, the .NET SDK requires Visual Studio 16.8 or newer. The SDK will not load in 16.7 and if you try to target `net5.0`, you will get compile errors. The error will be improved in a future release of 16.7 to advise you to update your Visual Studio installation. https://github.com/dotnet/msbuild/pull/5826

3. dotnet new -i for a template uses NuGet restore to download the template package and this requires a `TargetFramework`.  The template engine defaults to `netcoreapp1.0` to restore the template.  This will trigger warning NETSDK1138 as that target framework is no longer supported but has no negative affect on the template download.
