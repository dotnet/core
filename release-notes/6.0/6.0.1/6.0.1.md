...
## Known Issue 
There have been limited reports of a failure to install the update via Microsoft Update on Windows 11. The update fails with error code 0x80070643. A workaround is to install the package that is failing manually by downloading it from the .NET 6.0.101 x64 SDK download site, then scanning again. This issue is tracked on [dotnet/issue#7044](https://github.com/dotnet/core/issues/7044).
