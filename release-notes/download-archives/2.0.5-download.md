...
curl -O https://packages.microsoft.com/config/ubuntu/17.10/packages-microsoft-prod.deb
...
curl https://packages.microsoft.com/keys/microsoft.asc | gpg --dearmor > microsoft.gpg
sudo mv microsoft.gpg /etc/apt/trusted.gpg.d/microsoft.gpg
curl -O https://packages.microsoft.com/config/ubuntu/17.04/packages-microsoft-prod.deb
sudo dpkg -i packages-microsoft-prod.deb
```
...
sudo mv prod.repo /etc/zypp/repos.d/microsoft-prod.list
```
